...
class Contest(object):
    ...
    @view_config(context='..acl.Contest', name='edit', renderer='event/edit.mako', permission='edit')
    def edit(self):
    ...
    @view_config(context='..acl.Contest', name='edit', renderer='event/edit.mako', permission='edit', request_method='POST')
    def edit_submit(self):
        ...
        controls = self.request.POST.items()
        (schema, f) = ContestForm.create_form(request=self.request,
                action=self.request.current_route_url(), type='contest', origname=contest.disp_name)
        #del schema['otherrequests']
    ...
    @view_config(context='..acl.Contest', name='extrainfo', renderer='event/extrainfo.mako', permission='edit')
    def extrainfo(self):
        contest = self.context.contest

        e = {}
        e['name'] = contest.name
        e['tickets'] = m.Ticket.find_contest_tickets(contest.id)
        e['url'] = {}
        e['url']['manage'] = self.request.route_url('defcne.c', traverse=(contest.dc, contest.id, 'manage'))
        e['url']['edit'] = self.request.route_url('defcne.c', traverse=(contest.dc, contest.id, 'edit'))
        e['url']['extrainfo'] = self.request.route_url('defcne.c', traverse=(contest.dc, contest.id, 'extrainfo'))
    ...
    @view_config(context='..acl.Contest', name='extrainfo', renderer='event/extrainfo.mako', permission='edit', request_method='POST')
    def extrainfo_submit(self):
        ...
        e['tickets'] = []
        e['url'] = {}
        e['url']['manage'] = self.request.route_url('defcne.c', traverse=(event.dc, event.id, 'manage'))
        e['url']['edit'] = self.request.route_url('defcne.c', traverse=(event.dc, event.id, 'edit'))
        e['url']['extrainfo'] = self.request.route_url('defcne.c', traverse=(event.dc, event.id, 'extrainfo'))
